Dom_LM_Secchi_model_function <- function(forecast_date, model_id) {
  ...
  targets_lm_data <- targets_secchi |> left_join(model_drivers, by = c("datetime", "site_id")) |> arrange(site_id, datetime) |> group_by(site_id) |> mutate(secchi_yday = lag(secchi, 1)) |> ungroup() |> filter(complete.cases(secchi, secchi_yday, air_temperature, precipitation_flux, eastward_wind, northward_wind, SchmidtStability_Jm2_mean, Temp_C_mean))
  ...
  for(i in 1:length(focal_sites)) {  
    ...
    lm_fit <- lm(secchi ~ secchi_yday + air_temperature + precipitation_flux + eastward_wind + northward_wind + SchmidtStability_Jm2_mean + Temp_C_mean, data = site_target)
    ...
    for(d in 2:length(forecasted_dates)) {
      ...
      lm_preds <- predict(lm_fit, newdata = pred_df)
